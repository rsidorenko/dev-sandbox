"""E2E getUpdates failure on concrete httpx live stack (app + process, MockTransport)."""
...
def test_live_app_run_iterations_getupdates_fails_counters_no_offset_no_audit(
    fetch_fail_mode: str,
) -> None:
    ...
    async def main() -> None:
        ...
        async with httpx.AsyncClient(transport=transport) as ac:
            ...
            try:
                ...
                assert app.bundle.bundle.runtime.current_offset is None
                assert len(await app.bundle.bundle.composition.audit.recorded_events()) == 0
            finally:
                await app.aclose()
...
@pytest.mark.parametrize("fetch_fail_mode", ["http_error", "ok_false"])
def test_live_process_run_until_stopped_getupdates_fails_counters_no_offset_no_audit(
    fetch_fail_mode: str,
) -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        if request.url.path.endswith("/getUpdates"):
    ...
    async def main() -> None:
        ...
        async with httpx.AsyncClient(transport=transport) as ac:
            ...
            try:
                ...
                _assert_fetch_failure_summary(s)
                assert proc.app.bundle.bundle.runtime.current_offset is None
                assert len(await proc.app.bundle.bundle.composition.audit.recorded_events()) == 0
            finally:
                await proc.aclose()
...
@pytest.mark.parametrize("fetch_fail_mode", ["http_error", "ok_false"])
def test_live_offset_not_rolled_back_after_success_then_fetch_failure(fetch_fail_mode: str) -> None:
    ...
    async def main() -> None:
        ...
        async with httpx.AsyncClient(transport=transport) as ac:
            app = build_slice1_httpx_live_runtime_app("t", client=ac)
            try:
                s = await app.run_until_stopped(
                    app.bundle.bundle.control,
                    correlation_id=new_correlation_id(),
                    max_iterations=2,
                )
                assert s.fetch_failure_count == 1
                ...
                assert app.bundle.bundle.runtime.current_offset == 8
...
def test_live_fetch_failure_is_not_send_failure(use_app: bool) -> None:
    send_hits: list[None] = []
